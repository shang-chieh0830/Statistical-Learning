...
`anova()` function, which performs an _analysis of variance_ (ANOVA, using an F-test) in order to test the null
...
`plot.Gam()` method.  Conveniently, even though
  `gam1` is not of class `Gam` but rather of class `lm`, we can 
